...
const contentCache = {
    ...
    "control-flow-comparison.html": "条件分岐 制御構文 control flow if else elif elsif switch case match when for while loop 例外処理 try catch except finally rescue ensure throw raise break continue next redo retry 三項演算子 ガード節 早期リターン guard clause do-while until unless 論理演算子 AND OR NOT && || ! and or not 比較演算子 == === != !== > < >= <= インデント ループ制御 反復処理 配列 iterate enumerate forEach each map filter reduce JavaScript Ruby Python",
